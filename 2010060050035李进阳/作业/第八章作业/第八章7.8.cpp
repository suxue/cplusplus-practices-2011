...
int main()
{
	...
	cout<<"请输入技术人员"<<t1.GetName()<<"本月的工作时数:";
	int wt;
	cin>>wt;
	t1.SetworkHours(wt);
	t1.pay();
	
	cout<<"请输入销售经理"<<sm1.GetName()<<"所管辖部门的月销售总额:";
	float s;
	cin>>s;
	sm1.Setsales(s);
	sm1.pay();
	...
	cout<<"编号"<<m1.GetindividualEmpNo()<<m1.GetName()
	<<"本月工资"<<m1.GetaccumPay()<<endl;
	
	cout<<"编号"<<t1.GetindividualEmpNo()<<t1.GetName()
	<<"本月工资"<<t1.GetaccumPay()<<endl; 
	
	cout<<"编号"<<sm1.GetindividualEmpNo()<<sm1.GetName()
	<<"本月工资"<<sm1.GetaccumPay()<<endl; 
	
	cout<<"编号"<<s1.GetindividualEmpNo()<<s1.GetName()
	<<"本月工资"<<s1.GetaccumPay()<<endl; 
}
